...
[Functions]
  ...
  [./timestep_function]
    ...
    type = ParsedFunction
    value = 'if(T<5.6, 2e-4, 4e-4/(1+exp(0.1*(T-5.6))))' # if(t<0.07, 2e-3, 5e-4)
    vals = max_temp
    vars = T
  [../]
  [./outer_pressure_fct]
...
[Materials]
  [./no_mech]
    ...
    pressurization_coefficient = 5e-4 # 1e-7
    ar = 10
    total_porosity = 0.1
    temperature = temperature
    phi0 = 0.1
...
[Postprocessors]
  active = 'disp_hole_x avg_dissip max_r_theta new_timestep old_timestep max_temp Total_time'
  [./max_temp]
    type = NodalMaxValue
    variable = temperature
  [../]
...
[Outputs]
  ...
  file_base = Quarter_thickER_DP_slope0.5_timestep1
[]
